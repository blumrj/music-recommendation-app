...
import { albumEmbeddingService } from "./album-embedding.orchestrator";

interface AlbumWithEmbedding {
...
interface ClusterResult {
  ...
  albums: AlbumWithEmbedding[];
}

class AlbumClusteringService {
  private readonly K_CLUSTERS = 5;
   ...
   */
  private shuffleAndSelect(
    albums: Array<{ spotifyAlbumId: string; albumName: string; artist: string; imageUrl?: string }>,
    count: number
  ) {
  ...
  private async getAlbumEmbeddings(
    userAlbums: Array<{
      spotifyAlbumId: string;
      albumName: string;
      artist: string;
  ...
  ): Promise<AlbumWithEmbedding[]> {
    const result: AlbumWithEmbedding[] = [];
    let successCount = 0;
    let failCount = 0;
  ...
  private async kMeansClustering(albums: AlbumWithEmbedding[]): Promise<ClusterResult[]> {
    ...
    while (iteration < this.MAX_ITERATIONS && !converged) {
      ...
      if (iteration % 5 === 0 || iteration < 3) {
        console.log(`[CLUSTERING] Iteration ${iteration}: clusters:`, 
          clusters.map(c => c.albums.length).join(","),
          `movement: ${movement.toFixed(6)}`
        );
      }
  ...
  private computeTotalMovement(oldCentroids: EmotionalVector[], newCentroids: EmotionalVector[]): number {
    let totalMove = 0;
  ...
  private computeCentroidVariances(centroids: EmotionalVector[]): { averageDistance: number; maxDistance: number } {
    let totalDist = 0;
    let maxDist = 0;
  ...
  private initializeCentroids(albums: AlbumWithEmbedding[], k: number): EmotionalVector[] {
    const centroids: EmotionalVector[] = [];
  ...
  private assignToClusters(
    albums: AlbumWithEmbedding[],
    centroids: EmotionalVector[]
  ): ClusterResult[] {
    const clusters: ClusterResult[] = centroids.map((centroid, id) => ({
      clusterId: id,
      centroid,
      albums: []
    }));
  ...
  private computeCentroid(albums: AlbumWithEmbedding[]): EmotionalVector {
    if (albums.length === 0) {
      return {
        valence: 0.5,
        arousal: 0.5,
  ...
  private selectAnchors(clusters: ClusterResult[]): AlbumWithEmbedding[] {
    const anchors: AlbumWithEmbedding[] = [];

    for (const cluster of clusters) {
      if (cluster.albums.length === 0) continue;
...
export const albumClusteringService = new AlbumClusteringService();
