...
import { PrismaClient, AlbumEmotionalEmbedding } from "@prisma/client";
import {
  EmotionalVector,
  AlbumEmbedding,
} from "../../types/embedding.dto";
import { lastfmClient } from "../../infrastructure/lastfm/lastfm-client";
import { albumEnrichmentService } from "./album-enrichment.service";
...
class AlbumEmbeddingService {
   ...
   */
  private computeEmbeddingFromAudioFeatures(
    audioFeatures: Record<string, number>,
    metadata: { trackCount?: number; popularity?: number }
  ): EmotionalVector {
    // ─────────────────────────────────────────────────────────────────────
   ...
   */
  private computeConfidence(metadata: {
    popularity?: number;
    trackCount?: number;
    releaseDate?: Date;
  }): number {
   ...
   */
  private normalizeLoudness(loudnessDb: number): number {
    const MIN_DB = -60;
    const MAX_DB = 0;
  ...
  private async enrichWithLastfm(
    ...
    try {
      ...
      if (tags.length > 0) {
        ...
        const enrichmentResult = await albumEnrichmentService.enrichEmbedding(baseEmbedding, tags);
  ...
  async getOrComputeEmbedding(
    spotifyAlbumId: string,
    audioFeatures: Record<string, number>,
    metadata?: { albumName?: string; artist?: string; imageUrl?: string; spotifyUrl?: string; trackCount?: number; popularity?: number }
    ...
    if (metadata?.albumName && metadata?.artist) {
      ...
      embedmentResult = await this.createEmbeddingFromLastfm(
        metadata.albumName,
        metadata.artist
      );
    } else {
