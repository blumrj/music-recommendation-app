...
import { fastTextService, WordEmbedding } from "../../shared/embeddings/fasttext-embedding.service";
...
class SemanticSimilarityService {
  private dimensionEmbeddings: Map<string, WordEmbedding> = new Map();
  private tagSimilarityCache: Map<string, Partial<EmotionalVector>> = new Map();

  constructor() {
    // Pre-compute embeddings for all dimension names (fast, done once)
    this.precomputeDimensionEmbeddings();
    console.log(`[SEMANTIC] Service initialized with ${getDimensionNames().length} dimensions`);
  }
   ...
   * @private
   */
  private precomputeDimensionEmbeddings(): void {
    const dimensions = getDimensionNames();
    for (const dim of dimensions) {
      const embedding = fastTextService.embedWord(dim);
   ...
   */
  computeTagDimensionSimilarities(tag: string): Partial<EmotionalVector> {
    ...
    // Embed the tag
    let tagEmbedding: WordEmbedding;
    try {
      tagEmbedding = fastTextService.embedWord(normalizedTag);
    } catch (error) {
    ...
    const allSimilarities: Record<string, number> = {};
    const dimensions = getDimensionNames();
    ...
    for (const dim of dimensions) {
      ...
      try {
        const similarity = fastTextService.cosineSimilarity(tagEmbedding, dimEmbedding);
    ...
    const similarities: Partial<EmotionalVector> = {};
    ...
    if (sortedDims.length > 0 && sortedDims[0][1] !== undefined) {
      similarities[sortedDims[0][0] as keyof EmotionalVector] = sortedDims[0][1];
    }
    ...
    if (sortedDims.length > 1 && sortedDims[1][1] !== undefined && sortedDims[1][1] > SPARSIFICATION_THRESHOLD) {
      similarities[sortedDims[1][0] as keyof EmotionalVector] = sortedDims[1][1];
    }
   ...
   */
  getCacheStats(): { tagCache: number; dimensionCache: number } {
    return {
