...
import { artistExpansionService } from "./artist-expansion.service";
import { lastfmDiscoveryService } from "./lastfm-discovery.service";
...
/**
 * Album candidate from discovery
 */
...
/**
 * Clean album candidate ready for ranking
 */
interface CleanAlbumCandidate extends AlbumCandidate {
  spotifyAlbumId: string;  // Now required
}
...
class CandidatePoolService {
  ...
  async generateCandidatePool(
    ...
    targetPoolSize: number = 400
  ): Promise<CleanAlbumCandidate[]> {
    console.log(`\n${"═".repeat(70)}`);
    ...
    try {
      ...
      const seedArtists = await artistExpansionService.getSeedArtists(userId, 10);
      ...
      const discovered = await lastfmDiscoveryService.expandSimilarArtists(
        seedArtistNames,
        6,   // 6 similar artists per seed
      ...
      const withExploration = await explorationService.addExploratoryAlbums(
        discovered,
        targetPoolSize,
        explorationRatio
      );
      ...
      return withEmbeddings as CleanAlbumCandidate[];
    } catch (error: any) {
  ...
  private async filterCandidates(
    ...
    return highConfidence as CleanAlbumCandidate[];
  }
  ...
  private async ensureEmbeddings(
    ...
    for (let i = 0; i < candidates.length; i += BATCH_SIZE) {
      ...
      const results = await Promise.allSettled(
        batch.map(album =>
          albumEmbeddingService.getOrComputeEmbedding(
            album.spotifyAlbumId,
            {},  // No audio features (Last.fm-only)
            {
      ...
      for (let j = 0; j < batch.length; j++) {
        if (results[j].status === 'fulfilled') {
          ...
          failed++;
          console.warn(
            `[POOL-EMBED] Failed to embed "${batch[j].albumName}" by ${batch[j].artist}:`,
            (results[j] as PromiseRejectedResult).reason?.message || 'Unknown error'
          );
        }
      }
   ...
   */
  private countBySource(candidates: CleanAlbumCandidate[]): string {
