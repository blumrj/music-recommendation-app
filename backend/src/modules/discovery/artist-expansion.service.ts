...
interface SeedArtist {
  name: string;
  frequency: number;  // How many albums/surveys by this artist
  source: "saved" | "surveyed";
  lastSeen: Date;
}
...
class ArtistExpansionService {
   ...
   */
  private async getSurveyedArtists(userId: string): Promise<Map<string, Date>> {
    try {
   ...
   * // ]
   */
  async getSeedArtists(userId: string, limit: number = 4): Promise<SeedArtist[]> {
    ...
    try {
      ...
      const artistScores = new Map<
        string,
        { frequency: number; lastSeen: Date; source: string[] }
      >();
      ...
      for (const [artist, date] of savedArtists.entries()) {
        ...
        if (date > current.lastSeen) current.lastSeen = date;
      ...
      const now = Date.now();
      const seedArtists: SeedArtist[] = Array.from(artistScores.entries()).map(
        ([name, data]) => ({
          name,
          ...
          source: (data.source.includes("surveyed") && data.source.includes("saved"))
            ...
            : (data.source[0] as "saved" | "surveyed"),
   ...
   */
  static getArtistNames(seedArtists: SeedArtist[]): string[] {
   ...
   */
  static getTopByFrequency(seedArtists: SeedArtist[], limit: number = 5): string[] {
    return seedArtists
      .slice(0, limit)
      .map(a => a.name);
  }
   ...
   */
  static getTopByRecency(seedArtists: SeedArtist[], limit: number = 5): string[] {
    const sorted = [...seedArtists].sort(
...
export const artistExpansionService = new ArtistExpansionService();
