...
import { ContextModifier } from "../../types/embedding.dto";
import { normalizeModifier as sharedNormalizeModifier, addModifiers as sharedAddModifiers } from "../../shared/math/normalization";
...
class ContextModifierService {
   ...
   */
  private computeTimeOfDay(): "morning" | "afternoon" | "evening" | "night" {
   ...
   */
  private computeSeason(): "spring" | "summer" | "autumn" | "winter" {
   ...
   */
  private computePrecipitationIntensity(condition: string): number {
    if (condition === "Rainy" || condition === "Thunderstorm") return 0.8;
    if (condition === "Drizzle") return 0.3;
  ...
  async computeContextModifier(weatherData: {
  ...
  }): Promise<ContextModifier & { timeOfDay: string; season: string }> {
    ...
    // ─────────────────────────────────────────────────────────────────────
    const precipModifiers = this.computePrecipitationModifiers(
      precipitationIntensity
    );
    Object.assign(modifier, sharedAddModifiers(modifier, precipModifiers));

    // ─────────────────────────────────────────────────────────────────────
    // CLOUDINESS EFFECTS
    // ─────────────────────────────────────────────────────────────────────
    const cloudModifiers = this.computeCloudModifiers(weatherData.cloudiness);
    Object.assign(modifier, sharedAddModifiers(modifier, cloudModifiers));

    // ─────────────────────────────────────────────────────────────────────
    // TIME OF DAY EFFECTS
    // ─────────────────────────────────────────────────────────────────────
    const timeModifiers = this.computeTimeOfDayModifiers(timeOfDay);
    Object.assign(modifier, sharedAddModifiers(modifier, timeModifiers));

    // ─────────────────────────────────────────────────────────────────────
    // SEASONAL EFFECTS
    // ─────────────────────────────────────────────────────────────────────
    const seasonModifiers = this.computeSeasonalModifiers(season);
    Object.assign(modifier, sharedAddModifiers(modifier, seasonModifiers));
    ...
    Object.assign(modifier, sharedAddModifiers(modifier, compoundModifiers));

    // Normalize to valid range (uses shared implementation)
    const normalized = sharedNormalizeModifier(modifier);
   ...
   */
  private computeTemperatureModifiers(temp: number): ContextModifier {
   ...
   * @returns {ContextModifier}
   */
  private computePrecipitationModifiers(intensity: number): ContextModifier {
    const modifiers: ContextModifier = {};
   ...
   * @returns {ContextModifier}
   */
  private computeCloudModifiers(cloudiness: number): ContextModifier {
    const modifiers: ContextModifier = {};
   ...
   * @returns {ContextModifier}
   */
  private computeTimeOfDayModifiers(
   ...
   * @returns {ContextModifier}
   */
  private computeSeasonalModifiers(
   ...
   * @returns {ContextModifier}
   */
  private computeCompoundInteractions(weatherData: {
    temperature: number;
   ...
   * @returns {ContextModifier}
   */
  private addModifiers(a: ContextModifier, b: ContextModifier): ContextModifier {
   ...
   */
  private normalizeModifier(modifier: ContextModifier): void {
    const MAX_DELTA = 0.3;

    (Object.keys(modifier) as Array<keyof ContextModifier>).forEach((key) => {
      if (key === "confidence") return;
      
      const value = modifier[key] as number;
      if (typeof value === "number") {
        (modifier[key] as number) = Math.max(-MAX_DELTA, Math.min(MAX_DELTA, value));
      }
    });
  }
   ...
   */
  private computeContextConfidence(weatherData: {
    temperature: number;
    precipitationIntensity: number;
    cloudiness: number;
    humidity: number;
...
export const contextModifierService = new ContextModifierService();
