...
class SurveyController {
  ...
  async saveSurvey(req: Request, res: Response) {
    try {
      ...
      const survey = await surveyService.saveSurveyResponse({
        userId,
      ...
      const totalSurveys = await surveyService.getCompletedSurveyCount(userId);
   ...
   */
  async analyzeTaste(req: Request, res: Response) {
    try {
      // Extract user ID from JWT token (set by authMiddleware)
      const userId = (req as any).userId;
      ...
      const surveyCount = await surveyService.getCompletedSurveyCount(userId);
      ...
      const profile = await surveyService.generateTasteProfile(userId, surveys, user.spotifyToken);
   ...
   */
  async getAvailableAlbumsForSurvey(req: Request, res: Response) {
    try {
      ...
      const albums = await surveyService.getAvailableAlbumsForSurvey(
        user.spotifyToken,
        userId,
        15
      );
   ...
   */
  async getSurveyedAlbums(req: Request, res: Response) {
    try {
      // STAGE 1: Extract from request
      const userId = (req as any).userId;

      // STAGE 3: Call service (no validation needed, userId is guaranteed valid)
      const albums = await surveyService.getSurveyedAlbums(userId, 10);
