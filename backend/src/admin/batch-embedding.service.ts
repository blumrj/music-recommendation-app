...
import { albumEmbeddingService } from "../modules/embeddings/album-embedding.orchestrator";
...
class BatchEmbeddingService {
   ...
   * }
   */
  async batchEmbedAllAlbums(): Promise<{
    processed: number;
    created: number;
    cached: number;
    failed: number;
   ...
   */
  private async fetchAllAlbums(): Promise<
    Array<{ spotifyId: string; name: string; artist: string; imageUrl?: string }>
  > {
   ...
   */
  private getDefaultAudioFeatures(): Record<string, number> {
    return {
      danceability: 0.5,
      energy: 0.5,
      loudness: -5,
   ...
   * });
   */
  async batchEmbedAllAlbumsWithProgress(
