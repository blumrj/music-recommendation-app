...
export interface GenrePrior {
  ...
  arousal?: number;           // energetic ↔ calm
  tension?: number;           // tense ↔ relaxed
  warmth?: number;            // warm ↔ cold
  intimacy?: number;          // personal ↔ distant
  density?: number;           // layered ↔ minimal
  groundedness?: number;      // grounded ↔ dreamy

  // Base confidence for this genre (when used as signal)
  confidence: number;
}

export const genrePriors: Record<string, GenrePrior> = {
  // ROCK VARIANTS
  "indie rock": {
    warmth: +0.15,
    density: +0.15,
 ...
 */
export function getGenrePrior(genreName: string): GenrePrior | null {
  const normalized = genreName.toLowerCase().trim();
  return genrePriors[normalized] ?? null;
}
...
export function getAllGenres(): string[] {
  return Object.keys(genrePriors).sort();
}

/**
 * Check if genre has prior defined
 */
export function hasGenrePrior(genreName: string): boolean {
  const normalized = genreName.toLowerCase().trim();
  return normalized in genrePriors;
}
