 ...
 */
export const spotifyRefreshMiddleware = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    try {
      ...
      (req as any).spotifyToken = spotifyToken;
      logger.info("SPOTIFY", "✓ Spotify token attached to request");
      next();
    } catch (error: any) {
      logger.error("SPOTIFY", `Failed to get Spotify token: ${error.message}`);
      return res.status(401).json({ error: error.message });
    }
  } catch (error: any) {
    logger.error("SPOTIFY", `Middleware error: ${error.message}`);
