 ...
 */
export const EMOTIONAL_DIMENSIONS: Array<keyof EmotionalVector> = [
  "valence",
  "arousal",
  "tension",
  "warmth",
 ...
 */
export function cosineSimilarityGeneric(vec1: number[], vec2: number[]): number {
...
export function blendSignals(
  signals: Array<{ embedding: EmotionalVector | null; weight: number }>
): EmotionalVector {
  const result: Record<string, number> = {};
  const dims = EMOTIONAL_DIMENSIONS;
  ...
  for (const dim of dims) {
    ...
    for (const signal of signals) {
      if (signal.embedding && signal.weight > 0) {
        const val = signal.embedding[dim as keyof EmotionalVector];
        if (val !== undefined) {
  ...
  return normalizeVector(result as unknown as EmotionalVector);
}
 ...
 */
export function sharpenContrast(
  ...
  return normalizeVector(sharpened as unknown as EmotionalVector);
}
