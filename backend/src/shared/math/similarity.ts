...
import { cosineSimilarityGeneric, euclideanDistance, EMOTIONAL_DIMENSIONS } from "./vector";
...
export function cosineSimilarity(vec1: number[], vec2: number[]): number {
  return cosineSimilarityGeneric(vec1, vec2);
}
 ...
 */
export function pearsonCorrelation(values1: number[], values2: number[]): number {
 ...
 */
export function chebyshevDistance(vec1: number[], vec2: number[]): number {
  if (vec1.length !== vec2.length) {
    throw new Error("Vectors must have same length");
  }
 ...
 */
export function dimensionSimilarities(
  v1: EmotionalVector,
  v2: EmotionalVector
): Record<keyof EmotionalVector, number> {
  const result: Partial<Record<keyof EmotionalVector, number>> = {};
  ...
  return result as Record<keyof EmotionalVector, number>;
}
 ...
 */
export function harmonicMean(similarities: number[]): number {
  if (similarities.length === 0) return 0;
  if (similarities.includes(0)) return 0;
 ...
 */
export function maxPooling(similarities: number[]): number {
 ...
 * @returns Average similarity
 */
export function averagePooling(similarities: number[]): number {
  if (similarities.length === 0) return 0;
