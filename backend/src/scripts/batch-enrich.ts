...
import { albumEnrichmentService } from "../services/album-enrichment.service";
  ...
  try {
    ...
    for (let i = 0; i < embeddings.length; i++) {
      ...
      try {
        ...
        let tags = await lastfmClient.fetchAlbumTags(embedding.artist || "", embedding.albumName || "");
        if (tags.length === 0 && embedding.artist) {
          tags = await lastfmClient.fetchArtistTags(embedding.artist);
        }
