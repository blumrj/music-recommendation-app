...
import { SpotifyTokenResponse, SpotifyUserProfile } from "../types/spotify.dto";
...
export class AuthService {
  ...
  async getUserProfile(accessToken: string): Promise<SpotifyUserProfile> {
    try {
      ...
      const response = await client.get<SpotifyUserProfile>("/me");
  ...
  verifyRefreshToken(token: string): { userId: string } {
    try {
      ...
      const decoded = jwt.verify(token, JWT_REFRESH_SECRET as any) as { 
        userId: string 
      };
      return decoded;
    } catch (error) {
