...
/**
 * Album candidate for exploration
 */
...
class ExplorationService {
   ...
   */
  private readonly EMOTIONAL_TAGS = [
   ...
   */
  async addExploratoryAlbums(
    existingCandidates: AlbumCandidate[],
    ...
    try {
      ...
      const exploratoryTags = this.sampleRandomTags(
        Math.max(1, Math.floor(exploratoryCount / 20))  // ~20 albums per tag
      );
      
      console.log(`[EXPLORATION] Sampled ${exploratoryTags.length} random tags: ${exploratoryTags.join(", ")}`);
      
      // Get albums from random tags
      const exploratory = await lastfmDiscoveryService.expandByTags(
        exploratoryTags,
        Math.ceil(exploratoryCount / exploratoryTags.length)
      );
   ...
   */
  private sampleRandomTags(count: number): string[] {
   ...
   */
  private deduplicateCandidates(albums: AlbumCandidate[]): AlbumCandidate[] {
