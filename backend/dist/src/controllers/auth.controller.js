...
class AuthController {
    ...
    async login(req, res) {
        try {
            ...
            const authUrl = auth_service_1.authService.generateAuthUrl();
    ...
    async callback(req, res) {
        try {
            ...
            const spotifyTokens = await auth_service_1.authService.exchangeCodeForToken(code);
            // STAGE 4: Use Spotify access token to fetch user's Spotify profile
            const spotifyProfile = await auth_service_1.authService.getUserProfile(spotifyTokens.access_token);
            // STAGE 5: Create or update user in our database with Spotify data
            const user = await users_service_1.userService.createOrUpdateUser(spotifyProfile.id, spotifyProfile.email, spotifyProfile.display_name, spotifyTokens.refresh_token || "", spotifyTokens.access_token, spotifyTokens.refresh_token);
            ...
            const accessToken = auth_service_1.authService.generateAccessToken(user.id);
