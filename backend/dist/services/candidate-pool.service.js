...
class CandidatePoolService {
    ...
    async ensureEmbeddings(candidates) {
        ...
        for (let i = 0; i < candidates.length; i += BATCH_SIZE) {
            ...
            const results = await Promise.allSettled(batch.map(album => album_embedding_service_1.albumEmbeddingService.getOrComputeEmbedding(album.spotifyAlbumId, {}, // No audio features (Last.fm-only)
            {
