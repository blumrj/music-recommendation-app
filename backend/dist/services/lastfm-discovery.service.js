...
exports.lastfmDiscoveryService = void 0;
...
class LastfmDiscoveryService {
     ...
     */
    async getSimilarArtistsFromLastfm(artistName, limit = 15) {
        try {
            console.log(`[LASTFM] Getting similar artists for "${artistName}"...`);
            const response = await lastfm_client_1.lastfmClient.getSimilarArtists(artistName, limit);
     ...
     */
    async getTopAlbumsFromLastfm(artistName, limit = 10) {
        try {
            console.log(`[LASTFM] Getting top ${limit} albums for "${artistName}"...`);
            const response = await lastfm_client_1.lastfmClient.getTopAlbumsByArtist(artistName, limit);
            if (!response || !Array.isArray(response)) {
            ...
            const albums = response
                ...
                .filter((album) => album.albumName && album.albumName.length > 0);
     ...
     */
    async expandSimilarArtists(seedArtistNames, maxRelatedPerArtist = 15, maxAlbumsPerArtist = 10) {
        ...
        for (const seedArtist of seedArtistNames) {
            try {
                ...
                const similarArtists = await this.getSimilarArtistsFromLastfm(seedArtist, maxRelatedPerArtist);
                ...
                for (const relatedArtist of similarArtists) {
                    const topAlbums = await this.getTopAlbumsFromLastfm(relatedArtist, maxAlbumsPerArtist);
     ...
     */
    async expandByTags(emotionalTags, maxAlbumsPerTag = 20) {
        ...
        for (const tag of emotionalTags) {
            try {
                ...
                const response = await lastfm_client_1.lastfmClient.getTopAlbumsByTag(tag, maxAlbumsPerTag);
                if (!response || !Array.isArray(response)) {
                ...
                const albums = response
                    ...
                    .filter((album) => album.albumName && album.artist);
     ...
     */
    static deduplicateCandidates(albums) {
        const seen = new Set();
        const deduplicated = [];
     ...
     */
    static mergeCandidateSources(...sourceLists) {
